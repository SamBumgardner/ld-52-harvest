...
func _ready():
	...
	for button in $ButtonsContainer.get_children():
		...
		button.connect("pressed", Callable(self, "_on_LevelButton_pressed").bind(level_suffix))

func _on_LevelButton_pressed(level_suffix):
	get_tree().change_scene_to_file("res://gameplay/levels/Level_" + level_suffix + ".tscn")
